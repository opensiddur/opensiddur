...
# run the installer
RUN java -jar /tmp/eXist-db-setup-4.4.0-opensiddur.jar -console -options /tmp/docker-install-options.conf
...
RUN ln -s /usr/local/opensiddur/extensions/indexes/lucene/lib/hspell-data-files /usr/local/opensiddur/tools/yajsw/classes/hspell-data-files
